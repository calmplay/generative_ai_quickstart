# -*- coding: utf-8 -*-
# @Time    : 2024/10/15 22:40
# @Author  : cfushn
# @Comments: 条件辅助生成对抗网络（ACGAN）的定义文件，包含生成器、判别器，分类器以及训练逻辑
# @Software: PyCharm
...
class ACGAN:
    ...
    def train(self):
        ...
        fixed_labels = torch.randint(0, 10, (100,)).to(device)  # 随机生成 100 个类标签
        ...
        for epoch in range(epochs):
            ...
            print(
                f"\r ACGAN [Epoch {epoch + 1}/{epochs}] [D loss: {loss_discr.item():.3f}] "
            ...
            if (epoch + 1) % 10 == 0:
                ...
                with torch.no_grad():
                    ...
                    save_image(gen_imgs.data, f"./output/acgan_{epoch + 1}.png",
...
class _Generator(nn.Module):
    ...
    def forward(self, z, labels):
        ...
        z = z + labels_embed  # 将嵌入后的标签与噪声向量进行相加
...
class _Discriminator(nn.Module):
    def __init__(self, in_channels, n_classes):
        ...
        self.adv_layer = nn.Sequential(nn.Conv2d(512, 1, 4, 1, 0),
                                       nn.Sigmoid())  # 对抗性输出
        self.aux_layer = nn.Sequential(
            nn.Conv2d(512, n_classes, 4, 1, 0))  # 分类性输出

    def forward(self, img):
        features = self.conv(img)
        adv_output = self.adv_layer(features).view(-1, 1)
        aux_output = self.aux_layer(features).view(-1, 10)  # 10分类
